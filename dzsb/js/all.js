...
/**
 * Created by jaxer on 2017/1/19.
 */
$(function () {
    var index = 1;
    var autoSwitchE = setInterval(autoSwitch , 3000);
    function autoSwitch() {
        index ++;
        if(index == 6){
            index = 1;
    ...
    function switchHuan(dom) {
        var a = $(dom);
        a.siblings().removeClass("light");
        a.addClass("light");
        $("#ablity").text(a.data("desc"));

        $(".item2 .content .text").animate({"opacity": 0.1, "top": "80%"}, 10, "ease-out", function () {
            $(".item2 .content .text").animate({"opacity": 1, "top": "72%"}, 800);
        });
        if (a.hasClass("y1")) {
            $.each($(".huan"), function (index, item) {
                $(item).animate({
                    "left": $(item).data("voice-left") + "%",
                    "top": $(item).data("voice-top") + "%"
                }, 500, "ease-out")
            })
        }
        if (a.hasClass("y2")) {
            $.each($(".huan"), function (index, item) {
                $(item).animate({
                    "left": $(item).data("game-left") + "%",
                    "top": $(item).data("game-top") + "%"
                }, 500, "ease-out")
            })
        }
        if (a.hasClass("y3")) {
            $.each($(".huan"), function (index, item) {
                $(item).animate({
                    "left": $(item).data("video-left") + "%",
                    "top": $(item).data("video-top") + "%"
                }, 500, "ease-out")
            })
        }
        if (a.hasClass("y4")) {
            $.each($(".huan"), function (index, item) {
                $(item).animate({
                    "left": $(item).data("book-left") + "%",
                    "top": $(item).data("book-top") + "%"
                }, 500, "ease-out")
            })
        }
        if (a.hasClass("y5")) {
            $.each($(".huan"), function (index, item) {
                $(item).animate({
                    "left": $(item).data("media-left") + "%",
                    "top": $(item).data("media-top") + "%"
                }, 500, "ease-out")
            })
        }
    }
    $(".huan").click(function () {
        clearInterval(autoSwitchE);
        switchHuan(this);
